...
class LlamaAttentionXFormers(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
    ...
    def forward(
        ...
        attn_scale = self.head_dim ** -0.5

        query_states = query_states.reshape(bsz*self.num_heads, -1, self.head_dim).contiguous()
        key_states = key_states.reshape(bsz*self.num_heads, -1, self.head_dim).contiguous()
        value_states = value_states.reshape(bsz*self.num_heads, -1, self.head_dim).contiguous()
        import ipdb
        ipdb.set_trace()
        if attention_mask is not None:
            if attention_mask.size() != (bsz, 1, q_len, kv_seq_len):
